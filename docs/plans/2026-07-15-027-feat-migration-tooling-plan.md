...
- **KTD-5 — Grid ticks come from code analysis and seed with routes loaded, asserting the expected catalog diff.** The backfill moves *who holds what* (data); *which permissions each role gets* (ticks) comes from the code analyzer (R4). Because `Role#permission_keys=` silently drops keys not in the route-derived catalog, the generated seed must run under loaded routes and assert `expected - CurrentScope.catalog.keys == []`, failing loudly on a drop. This coupled to the companion silent-drop fix (plan 002, **issue #20** — not #21, which is break-glass). **That fix has landed** (PR #52): `Role#permission_keys=` now REJECTS a non-catalog key rather than dropping it, so the save fails loudly and the seed can lean on the engine instead of asserting the diff itself. Note the seed must therefore not hand it stale keys expecting a silent scrub — that is now an explicit `assign_permission_keys(keys, scrub: true)`.
...
- **#20 — `permission_keys` silent drop (plan 002). CLOSED, shipped in PR #52.** (This entry previously said "#21", which is break-glass — a misattribution that was invisible because both issues were closed and the answer coincided.) U4's grid seed (R9/KTD-5) depended on the drop being observable, and it now is: `Role#permission_keys=` rejects a non-catalog key and the save fails. The seed leans on the engine; it does not need to assert the catalog diff itself, and it must not assume a silent scrub — that now requires an explicit opt-in.
