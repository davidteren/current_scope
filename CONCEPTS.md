...
**Report mode**
The rehearsal posture: the gate still decides, but a refusal is downgraded to an observation, so the request proceeds and the would-be denial is recorded instead. It exists so a host can discover which grants it needs from real traffic before any user is refused, and flipping out of it to enforcing is the last step of adoption.
