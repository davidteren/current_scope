...
- **Goal:** the security fix. Closes consequences 1 and 2.
...
- **Approach:** `decide` gains `model:` (default `nil`) and forwards it. In `record_less_scoped_grant?` (`resolver.rb:304-311`), resolve the type from **two** sources: `record` when `record.is_a?(Class)` (R5 — the class form already carries it), else the `model:` kwarg. Unknown → `return false` (R3). Known → add `resource_type: type.base_class.name` to the query (R6) and switch `roles_ticking` → `roles_granting` (R4/KTD-3). Rewrite the method's comment block: it currently explains why the branch *cannot* bind (`resolver.rb:282-294`) and that reasoning is now inverted — state the new rule, and why `roles_granting` is safe *here specifically*.
...
- **Patterns to follow:** `scope_for`'s `resource_type: model.base_class.name` (`resolver.rb:86-90`) — the query shape to mirror; `scoped_grant?` (`resolver.rb:247-255`) for the bind-then-`roles_granting` idiom.
- **Test scenarios:**
  ...
  - **Closes consequence 2:** scoped `full_access` on `Report#1`, `model: Report` → `reports#index` **allowed**, and `scope_for(Report)` returns the same record — gate and list now agree.
...
- **Requirements:** R7, R9.
- **Dependencies:** U2.
- **Files:** `lib/current_scope/configuration.rb`, `lib/current_scope/guard.rb`, `test/integration/dev_diagnostics_test.rb`.
- **Approach:** add `config.warn_on_undeclared_collection_model`, defaulting to `diagnostics_default_on?` (`configuration.rb`, the `#41` env-aware helper). Two distinct nudges, both log-only, both at the Guard seam (never on advisory `allowed_to?`):
  ...
  2. **R9 inert hook:** `current_scope_model` declared while `current_scope_record` is not (`record.equal?(NO_RECORD)`). The declaration can never take effect. Fires regardless of the decision — it is a misconfiguration, not a denial.
...
- **Test scenarios:**
  ...
  - Flag off → no nudge, and `scoped_grant_exists?` is never called (flag-first).
...
- **Files:** `test/dummy/config/routes.rb`, `test/dummy/app/controllers/projects_controller.rb` (new), `test/dummy/app/controllers/documents_controller.rb` (new), `test/dummy/app/models/receipt.rb` (new — an STI **sibling** of `Invoice`, R6a), `test/dummy/app/controllers/nested_reports_controller.rb`.
...
- **Verification:** the `main` probe's bare-form `true` survives U2 on declared controllers; `permissions.rb:5-6`'s promise ("a view can never disagree with the controller gate") holds for every declared controller.
...
**In scope:** the `current_scope_model` hook; the record-less branch's type bind and fail-closed default; bounded `full_access`; the advisory path's matching bind (U6 — not optional, see Risks); the two nudges; the dummy controllers; docs.

**Deferred to Follow-Up Work:**
- **Simplifying `roles_ticking`.** After U2 its only caller is `roles_granting` (`resolver.rb:161`), where its `full_access` exclusion is a documented no-op (`resolver.rb:178-180`). Leaving it untouched keeps this PR's diff on the security path minimal; collapsing it is a separate, deliberate change.
...
- **Re-opening #49's P0 (highest).** KTD-3 permits `roles_granting` in this branch. If an implementer lands `roles_granting` without the `resource_type:` filter in the same query, one scoped `full_access` grant opens every record-less gate in the host app. Mitigation: the Verification Contract mutation-tests exactly this; U2's scenarios include the cross-type `full_access` denial; the method's comment must state the bind as the precondition.
