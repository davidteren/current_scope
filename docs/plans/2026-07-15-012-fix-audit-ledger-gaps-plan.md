...
- **R3.** Recording never requires an ambient controller context: `grant!` supplies **both actor and subject** explicitly (self-attributed to the grantee — R2/KTD-2). `Event.record!` gains optional `actor:` and `subject:` overrides that, when non-nil, are used instead of the ambient reads; when omitted, behavior (including the actorless **raise** and the `Current.user || actor` subject derivation) is byte-for-byte unchanged.
...
- **KTD-3 — `Event.record!` gains *additive*, non-nil `actor:` and `subject:` overrides; the actorless raise is preserved.** Signature becomes `record!(event:, target:, details: nil, actor: nil, subject: nil)`. When both are nil (the default, every existing caller), the method reads the ambient actor (raising if absent — **unchanged**) and derives `subject = Current.user || actor` exactly as today. When `actor` is non-nil, it is used and the ambient raise is skipped; when `subject` is non-nil, it is used instead of the ambient derivation. **Both overrides must be pinned together** for a self-attributed grant: a lone `actor:` override still lets the `subject` line fall through to the ambient `Current.user`, so a `grant!` issued from inside a request (a host's own "make owner" admin action, where the Context hook set `Current.user` to the acting admin) would record `actor == grantee` but `subject == admin` — subject ≠ actor, which the ledger contract (`event.rb:8-13`) reads as an **impersonation** row and attributes the grant backwards. The `subject:` override is what guarantees `actor == subject == grantee` (KTD-2) for *every* caller, not just the actorless ones. This is the minimum change that lets `grant!` self-attribute without weakening fail-closed for any other caller. Does not touch the SoD break-glass Guard recorder (which passes neither override and uses the ambient path).
...
- **Approach:** change the signature to `record!(event:, target:, details: nil, actor: nil, subject: nil)`. Directionally: `actor ||= CurrentScope::Current.actor`, then keep the existing `if actor.nil? … raise` block; then `subject ||= CurrentScope::Current.user || actor` (the explicit override wins, else the ambient derivation is byte-for-byte as today). The explicit `subject:` is what makes self-attribution hold **regardless of ambient context**: with only an `actor:` override the `subject` line would fall through to `Current.user`, so a `grant!` issued from inside a request (an app's own "make owner" admin action, where the Context hook set `Current.user` to the acting admin) would record `actor = grantee` but `subject = admin` — subject ≠ actor, which the ledger contract (`event.rb:8-13`) reads as an **impersonation** row, mis-attributing the grant. Pinning `subject:` closes that. `request_id` continues to read `CurrentScope::Current.request_id`. No other change to the method body, the `:strict` rescue, or the missing-table handling.
...
- **Test scenarios:**
  ...
  - Explicit `actor:` **and** `subject:` with an ambient `Current.user` set to a *different* user → the row uses the overrides, not the ambient user: `actor == subject == override` (proves self-attribution survives an ambient request context — the impersonation-mis-record guard; input: `Current.user = admin`, `record!(event: "x", target: role, actor: user, subject: user)` → `subject == user.to_gid`, not `admin`).
...
- **Approach:** in `grant!`, resolve the existing assignment's `prior_role` before writing (the row is a `find_or_initialize_by(subject:)`, so read `assignment.role` while it's still the old value). Wrap the `update!` and the recording in a `RoleAssignment.transaction`. After the `update!`, branch exactly as the controller does (KTD-4):
  - `prior_role.nil?` → `Event.record!(event: "org_role.assigned", target: subject, details: { role: role.name, source: "bootstrap" }, actor: subject, subject: subject)`
  - `prior_role.id != role.id` → `Event.record!(event: "org_role.changed", target: subject, details: { from: prior_role.name, to: role.name, source: "bootstrap" }, actor: subject, subject: subject)`
  - same role → no event.
  Both `actor:` and `subject:` are pinned to the grantee on every call, so the row is `actor == subject == grantee` (KTD-2) even if the host called `grant!` from within a request where `Current.user` is a different acting admin — otherwise the ambient user would leak into `subject` and the grant would mis-record as an impersonation row (see KTD-3).
...
- **Test scenarios:**
  ...
  - **Self-attributed under an ambient user:** with `Current.user` set to a *different* admin (simulating `grant!` called from within an admin request), `grant!(user)` still records `actor == subject == user` (the grantee), **not** the ambient admin — the row is not a spurious impersonation. Regression guard for the ambient-context mis-attribution (KTD-3).
...
- **SoD break-glass plan (`docs/plans/2026-07-12-001-feat-sod-bypass-breakglass-plan.md`):** that feature records `sod.bypassed` via `Event.record!` on the **ambient** actor path. U1's `actor:`/`subject:` overrides are additive and default-nil, so the break-glass recorder (which passes neither) is unaffected — verify the two land without a signature clash (both call `Event.record!` with keyword args; the added `actor:`/`subject:` are optional).
