...
- **R6.** The stock chrome carries a host back-link: `config.ui_home_path` is tri-state — an explicit path string → use it; `false` → render **no** back-link (the escape hatch for a host that *has* a root route but wants no link); `nil` (default) → auto-detect `main_app.root_path` and render the link **only if the host defines a root route**; render nothing otherwise. It must **never raise** on a host without a root path (fail-safe, not fail-closed — this is chrome, not a gate).
...
- **Approach:** add `current_scope_theme` to the existing `CurrentScope::ApplicationHelper` — lift the layout's line-1 ternary verbatim (`%w[light dark].include?(cookies[:current_scope_theme]) ? cookies[:current_scope_theme] : nil`) so behavior is identical, returning `"light"`/`"dark"`/`nil`. In `configuration.rb`, add `attr_accessor :ui_brand_name` (default `"CurrentScope"`) and `attr_accessor :ui_home_path` (default `nil`) in `initialize`, each with a doc comment matching the file's existing style and stating the honest scope (branding/chrome only, never a gate). `ui_home_path` is **tri-state** — a path string, `false` (suppress the back-link even when a root route exists), or `nil` (default → auto-detect) — so its doc comment must spell out all three states, following the file's existing tri-state idiom (`audit`, `subject_label`); `nil` reading as "auto-detect and show" rather than "off" is exactly the surprise the `false` state removes. No writer guard (contrast `allow_mutations_while_impersonating` — these are cosmetic; note that in the comment).
...
- **Test scenarios:**
  ...
  - `config.ui_home_path` defaults to `nil`; assignable and read back — including `false` (the explicit "no back-link" state) and a path string, proving all three states round-trip.
...
- **Test scenarios:**
  ...
  - **Back-link disabled:** `config.ui_home_path = false` with a root route present → no back-link rendered (the `false` opt-out beats auto-detect).
