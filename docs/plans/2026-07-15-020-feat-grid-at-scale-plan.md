...
- **R4.** Folding is controlled by `config.permission_grid_fold_custom_actions` (default **true**). Set false to restore the pre-#38 **column axis** — one-off actions reappear as columns, so column counts and blank-cell layout match pre-#38 (escape hatch for hosts asserting exact column counts). The flag governs the **column axis only**: the additive group-header `title` (R5) and namespace **row ordering** (R6) still apply unconditionally, so `= false` does **not** restore pre-#38 *row order* — see R8 and the Open Question on row-order-asserting hosts. Group columns and shared-leftover columns are unaffected by the flag.
...
- **Approach:** header (line 38-39): when `column.group`, add `title: column.actions.join(", ")` (or `"#{label} = #{actions.join(', ')}"`); leave single-action columns untitled (R5). In each row (after the fixed cells, line 78), when `grid.custom_actions_for(controller).any?`, render a native `<details>`/`<summary>` whose summary reflects **grant state, not a bare count** — e.g. `custom actions — 2 of 9 granted`, from `custom_actions_for(controller).count(&:checked)` of `.size` — and mark the summary (and/or the row header) when any folded cell is checked (e.g. a `cs-has-grant` class / dot) so an admin scanning a *collapsed* role can see which controllers carry custom grants without expanding every disclosure (the grid is a review surface, not just an edit surface — R1). The `<details>` contains one `<label><%= check_box_tag cell.name, cell.value, cell.checked … %></label>` per folded cell — reusing the exact cell markup from lines 63-69. No JS dependency for correctness (native disclosure; progressive-enhancement consistent with the `data-cs-row-all` note at line 47). CSS: a compact disclosure block styled to sit under the row, with a muted marker style for the granted state; keep CSP-safe, no web fonts (matches `application.css` house style). Ponytail: reuse `<details>`, no custom toggle JS; the granted count is a plain `count(&:checked)` — no new state.
...
- **Test scenarios:**
  ...
  - A controller with a one-off action renders a `<details>` whose summary reflects grant state and whose checkbox has `name="role[permission_keys][]"` and `value="controller#action"` (R1/R2).
  - Collapsed summary distinguishes some-granted from none-granted: a controller with 2 of its 9 folded one-offs granted renders a `2 of 9`-style summary and the `cs-has-grant` marker; the same controller with none granted renders `0 of 9` and no marker (R1 at-a-glance auditability).
...
- **Namespace ordering surprise (R6/KTD-5):** ordering by `[base_resource, full_path]` co-locates namespaced siblings but reorders the whole list (e.g. `dashboard`/`sessions` interleave differently). Accepted as the smallest fix that delivers the finding; confirm the maintainer prefers this over keeping the flat alphabetical order and *only* adding an indent/badge (which leaves siblings scattered). Documented as a non-breaking presentation change either way. **Row-order-asserting hosts:** the `fold_custom_actions = false` escape hatch restores the column axis but **not** row order (R6 is unconditional per R8), so a host whose tests assert exact row order still breaks with the flag off and currently has no opt-out. Confirm this is acceptable as a non-breaking presentation change, or decide the reorder needs its own mitigation — gate R6 behind its own flag (or reuse `fold_custom_actions` for a full byte-for-byte pre-#38 restore) if exact row order is a promised contract. Assumed acceptable-without-flag here.
