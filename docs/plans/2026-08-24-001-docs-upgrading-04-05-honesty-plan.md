...
- R1b. That same step list adds one scoped clause for MySQL hosts on the default `schema_format = :ruby`: `db:test:prepare` loads `schema.rb`, `schema.rb` cannot carry a MySQL collation, so the test columns come out case and accent insensitive and the guard still refuses to boot. Those hosts must also run `RAILS_ENV=test bin/rails current_scope:repair_schema`. The clause names its own exception: a host on `config.active_record.schema_format = :sql` builds the test database from `structure.sql`, which carries the collation, and does not need it. See KTD3 for the evidence. **Correction made during implementation (commit `fcb0759c`): `schema.rb` can carry a MySQL collation, so this bullet's flat "cannot" is wrong.** activerecord's `mysql/schema_dumper.rb` defines `schema_collation`, which emits a per-column `collation:` whenever a column's collation differs from its table's. The #151 migration collates the column while the table keeps the server default, so a dump taken from MySQL does carry it; a dump taken from PostgreSQL or SQLite carries none. The requirement itself stands: the shipped clause is now conditional on the adapter the dump came from, and it still routes MySQL hosts to `RAILS_ENV=test bin/rails current_scope:repair_schema`, which is idempotent and settles the question either way.
