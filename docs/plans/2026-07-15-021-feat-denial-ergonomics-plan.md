...
- **Approach:** in `current_scope_denied`, before/after setting the header, emit `Rails.logger&.info("[CurrentScope] denied #{exception&.permission} (#{exception&.reason}) → 403")`. Guard `exception` being nil (the method signature allows `exception = nil`). Keep it INFO — a denial is expected control-flow, not an error (KTD-4). Do not remove or change the header line.
  ...
  > **Coupling note (resolved):** **#23 landed first** (PR #55). `current_scope_denied` is already restructured — it writes the reason header and delegates the body to **`current_scope_render_denied(reason)`** (note the name: it is `current_scope_`-prefixed, not the bare `render_access_denied` this plan predicted, because a bare name is one a host controller could already own — and it takes the reason, because an overrider rendering a body has to know which denial it is answering). Add the log line inside that restructured method.
...
- **No change to the denial body.** Whether a denial renders a blank `head :forbidden` or a page is #23/#24's concern, not this one — and **#23 has since answered it** (PR #55): a *host* denial stays a bodyless `head :forbidden`; the engine's console overrides the seam to render an explanation page for its own front door. This issue still renders nothing new.
...
- **#23 — CLOSED, shipped in PR #55 (plan 005). The recommended order happened: #23 went first.** It added the new raise site (`require_full_access!` raising `reason: :not_full_access`), added that reason to the `AccessDenied` vocabulary comment, and restructured `current_scope_denied` — which now writes the header and delegates the body to **`current_scope_render_denied(reason)`**. Two corrections to what this plan predicted, both from #55's review: the seam is `current_scope_`-prefixed (a bare `render_access_denied` is a name a host controller could already own, and Ruby would silently call theirs), and it **takes the reason** (an overrider that renders a body must know which denial it is answering — without that, #55's own override answered the impersonation gate with the full-access page). So: build on the restructured method as it actually is, not as described here. #23's raise site still inherits `#permission` for free once this issue lands — KTD-1's point survives intact.
- **#24 (document denial behavior end-to-end) — `docs/plans/2026-07-15-006-docs-denial-behavior-plan.md`.** Docs-only; deliberately leaves `current_scope_denied` unchanged and documents the *current* blank-403 body + override seam + the `rescue_from StandardError` shadowing trap. This issue adds the *code* behind part of what #24 describes (new accessors, 403 classification, log line). **U4 must coordinate with #24's README section** so the two don't duplicate: #24 owns "what a denial renders and how to override it"; this issue owns "the accessors on the exception and the escaped-denial classification." #24's stop-condition (a) punted the "blank 403 is wrong" fix to #23/#39 — **#23 has since landed and answered the body half** (host denials stay bodyless; the engine console renders a page), so this plan is the remaining #39 half: *classification and diagnosability*, not the body.
