...
3. **`skip_before_action` inherits into subclasses — fails OPEN (major, security, docs-gap).** `skip_before_action :current_scope_check!, only: [:dashboard]` on `LegacyController` is inherited by `LegacySubController < LegacyController`, so `legacy_sub#dashboard` is ungated for any signed-in user — while the grid still shows it as a grantable, apparently-enforced permission. This is vanilla Rails callback-inheritance, but in an authorization gem the fail direction is *open*, and `GatingTripwire` cannot catch it (the gate legitimately did not run). The README's skip recipe (`README.md:90-96`) carries no inheritance warning.
...
- **KTD-4 — Skip-inheritance is framed as the guide's headline SECURITY warning, not a footnote.** It is the one trap that fails *open* and that `GatingTripwire` cannot catch (`gating_tripwire.rb:44` returns early when `@current_scope_checked` is set, and it never is on the inherited-skip subclass — the gate legitimately didn't run, so there's nothing to trip). The safe patterns (leaf-only skips, or re-assert `before_action :current_scope_check!` in the subclass) are prescriptive, with the "prefer leaf" default called out. Pairs with #37 (report-only mode) which would surface the ungated surface at runtime rather than relying on the reader.
...
- **Approach:** structure the page as:
  ...
  5. **The skip-inheritance trap — READ THIS (R4, KTD-4).** Headline security warning. Symptom: `skip_before_action :current_scope_check!, only: [:dashboard]` on a base controller silently ungates `subclass#dashboard` for any signed-in user; a roleless user gets 200 where they should get 403. Mechanism: Rails inherits the skip; the gate never runs, so `GatingTripwire` can't catch it and the grid still shows the action as enforced. Safe patterns: **prefer skips on leaf controllers**; if a base must skip, re-assert `before_action :current_scope_check!` in each subclass that should stay gated. Cross-link #37.
...
- **Patterns to follow:** the README's voice — symptom-then-mechanism-then-fix, blockquote callouts for foot-guns, fenced Ruby for recipes. Match the "residual foot-gun" blockquote style already in `README.md:154-163`. Repo-relative links only.
