...
- **Stop conditions — surface rather than guess if:**
  ...
  - (b) making a claim on the site accurate would require **changing engine behavior** rather than rewording — it must not; the fix is to describe what the engine does today (which the README already does correctly).
...
- **Patterns to follow:** the README's own wording — the meta description on this very page, and README's "they ask the same resolver" (`README.md:34`) and "View/gate disagreement is by design" (`README.md:490-494`) — so the site and README say the same thing.
