...
- **KTD-4 — The explanation page renders with `layout: false` (no engine chrome).** The engine layout (`layouts/current_scope/application.html.erb`) hardcodes a "full access" badge and a nav whose every link would itself 403 for this visitor. Rendering the denial inside that shell would be misleading and offer only dead links. A small self-contained view with `status: :forbidden, layout: false` is both lazier and less astonishing. (Directional: a one-paragraph page, no new CSS pipeline dependency.)
...
- **Approach:**
  ...
  - In the engine `ApplicationController`, override `render_access_denied` to `render "current_scope/errors/forbidden", status: :forbidden, layout: false`.
