...
The **gate** and the **scoped list** are the two halves of every per-record feature: the gate decides whether an action runs at all, the list decides which records it operates on. Both ask the **resolver**, so they read the same grants — but they bind those grants differently, and only the gate is enforced. For the collection-read actions the host lists (`collection_read_actions`, `index` by default) the gate derives its answer from the scoped list itself, so the two are one query there and agree by construction. Everywhere else they can still bind differently — treat "the gate let them in" and "it is in their list" as separate claims.
...
**Gate**
The enforcement point: a fail-closed check that runs before every action and refuses it unless the resolver allows it. The gate **admits** — it decides whether an action runs. It does not filter what that action then does, so it cannot narrow a list the action builds for itself. For a listed collection read it *derives* that admission from the scoped list — record membership decides whether the door opens — but it still only admits or refuses; narrowing remains the list's job.

**Scoped list**
The list-side companion to the gate: the set of records a subject may act on, derived from the same grants the gate reads. It **narrows**. Unlike the gate it is advisory — the host chooses to use it — so an action that ignores it and fetches everything will show everything to anyone the gate admitted. For listed collection reads it is also what the gate's admission derives from, so an empty list and a refused gate are the same fact there.
...
**Report mode**
The rehearsal posture: the gate still decides, but it lifts exactly one wall, "nobody has granted this yet". That refusal is downgraded to an observation, so the request proceeds and the would-be denial is recorded instead. Every other refusal survives report mode and still refuses: the separation-of-duties veto, an SoD action whose blind spot means the veto never ran, an undeclared collection type, and the console itself. None of those is fixed by granting, which is why they can never clear from the survey. Report mode exists so a host can discover the grants it needs from real traffic before any user is refused, and flipping out of it is the culminating step of the rollout rather than its end.

Two properties decide whether that rehearsal can be trusted, and both surprise people. It observes only what it can attribute: a request that reached the gate before authentication resolved a subject is downgraded and recorded **nowhere**, and that is the first class of request to be refused after the flip. And what it records is history, not state: the ledger is append-only, so the row that recorded a denial outlives the grant that fixed it. "Everything ever denied" and "what would still be denied" are different questions, and only the second one has an answer that can reach zero.
