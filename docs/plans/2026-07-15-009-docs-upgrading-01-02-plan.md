...
- **Approach (directional):**
  - In `engine.rb`, add `config.after_initialize do |app| … end`. Guard the whole block on `app.config.eager_load` (else host models aren't enumerable — KTD-4, Stop condition (a)) **and** `CurrentScope.config.sod_actions.empty?` **and** the silence knob being on. Only then scan for SoD-shaped models: `ApplicationRecord.descendants.select { |m| m.instance_methods(false).include?(:current_scope_initiator) || m.private_instance_methods(false).include?(:current_scope_initiator) }` (respond-to on the *class'* own methods, not inherited — directional; the exact predicate is an implementation detail). If any exist, `Rails.logger.warn` once with a message naming the finding and linking `UPGRADING.md`.
