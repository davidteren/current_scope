...
- Before the PR is opened: the follow-up issue for the five `SchemaGuard` boot messages that name no database (`lib/current_scope/schema_guard.rb:73`, `:81`, `:91`, `:113`, `:144`) exists, and its number is written into the "Deferred to follow-up work" bullet and the PR body.
