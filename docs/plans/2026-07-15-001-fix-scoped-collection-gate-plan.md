...
- **Test scenarios:**
  ...
  - **SoD unaffected:** a record-less SoD-action target still resolves SoD first (the new branch is downstream of the veto).
...
- **OQ-2 (which collection actions?):** The record-less branch opens *any* collection/class-form gate whose scoped role ticks the key, including `create` or a bulk action. Assumed acceptable and consistent with org grants (the role author controls the ticks; there is no record filter on `create` anyway). Confirm before release, or restrict to a read-ish allowlist as a follow-up (OQ-2 → Deferred).
