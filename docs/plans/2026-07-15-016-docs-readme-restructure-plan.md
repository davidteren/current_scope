...
4. **The real limitation is buried (discoverability).** The deferred resource-hierarchy/cascade limitation lives, fully explained, in `docs/ROADMAP.md:72-79` (§2.3), but the README surfaces it only as a parenthetical inside `scope_for` prose (`README.md`'s "Scoping a list" section ("parent/child cascade is deferred")) and never links ROADMAP. There is no "Limitations" heading.
