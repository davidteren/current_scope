...
- **Approach:** in `derive`, after building the route-derived `routed` keys (current lines 22-31), append synthetic keys when `CurrentScope.config.allow_sod_bypass`. Directionally:
  ...
  - Build `grouped_by_controller` from `routed` (`key.split("#") => controller, action`); select controllers whose action set intersects `CurrentScope.config.sod_actions`; for each, add `"#{controller}##{bypass_action}"`.
